...
/// The identifiers, pinned to what is actually registered with Apple.
///
/// These are derived by string interpolation from a prefix, which means a
/// rename can miss them: `"\(prefix).fits"` contains no literal "com.leejiles.fits"
/// for a search to find. Both the app group and the bundle IDs survived the
/// Fits → Uploadable rename that way, and a wrong App Group fails *silently* —
/// the app and the extension would simply stop sharing one export count, each
/// keeping its own, with nothing to see until a user reported free exports that
/// never ran out.
struct IdentifierTests {

    @Test func theIdentifiersMatchWhatIsRegisteredWithApple() {
        #expect(BundleConfig.appBundleID == "com.leejiles.uploadable")
        #expect(BundleConfig.shareBundleID == "com.leejiles.uploadable.share")
        #expect(BundleConfig.appGroupID == "group.com.leejiles.uploadable")
        #expect(BundleConfig.proProductID == "com.leejiles.uploadable.pro")
    }

    /// The extension's bundle ID must sit under the app's, or the App Group
    /// entitlement pairing is not what the portal was configured for.
    @Test func theExtensionLivesUnderTheApp() {
        #expect(BundleConfig.shareBundleID.hasPrefix(BundleConfig.appBundleID + "."))
        #expect(BundleConfig.appGroupID == "group.\(BundleConfig.appBundleID)")
    }

    /// The Keychain mirror is scoped to that same group, which is the whole
    /// mechanism by which one count is shared rather than two kept.
    @Test func theKeychainScopeFollowsTheRenamedGroup() {
        #expect(ExportStore.query()[kSecAttrAccessGroup as String] as? String
                == "group.com.leejiles.uploadable")
        #expect(ExportStore.query()[kSecAttrService as String] as? String
                == "group.com.leejiles.uploadable")
    }
}
